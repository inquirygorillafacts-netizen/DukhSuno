...
import type { DukhSunoSession } from '@/types';
import React from 'react';

export default function SunneCallsPage() {
  const { user } = useAuthStore();
  const [sessions, setSessions] = useState<DukhSunoSession[]>([]);
  ...
  useEffect(() => {
    ...
    const unsubscribe = onSnapshot(qAll, (snapshot) => {
      const allSessions = snapshot.docs.map(doc => ({ id: doc.id, ...doc.data() } as DukhSunoSession));
  ...
  const renderCallCard = (session: DukhSunoSession) => {
    const isMissed = session.status === 'missed' || session.status === 'rejected';
    return (
      <div key={session.id} className="glass bg-white dark:bg-white/5 p-5 rounded-[2.5rem] border border-white dark:border-white/10 flex items-center gap-5 hover:shadow-xl transition-all active:scale-[0.98] group">
        ...
        <div className="flex-1 min-w-0">
          <div className="flex justify-between items-start mb-0.5">
            <h4 className="font-bold truncate text-lg text-slate-800 dark:text-white uppercase tracking-tighter">{session.speakerDisplayName || 'Anonymous'}</h4>
            <p className="font-black text-lg text-[#ff4d6d]">{session.earnings ? `₹${session.earnings}` : '₹0'}</p>
          </div>
          <div className="flex items-center gap-3">
            <p className="text-[10px] text-slate-400 font-black uppercase tracking-widest">
              {session.duration ? `${Math.floor(session.duration / 60)} min` : 'Missed'} · {session.createdAt?.seconds ? new Date(session.createdAt.seconds * 1000).toLocaleDateString([], { month: 'short', day: 'numeric' }) : 'Recently'}
            </p>
            {session.rating && (
...
const StatSmall = ({ label, value, color }: { label: string, value: string, color: 'slate' | 'emerald' }) => (
  <div className="glass bg-white dark:bg-white/5 p-6 rounded-[2rem] border border-white dark:border-white/10 shadow-sm text-center">
    <p className={`text-3xl font-black tracking-tighter ${color === 'emerald' ? 'text-emerald-500' : 'text-slate-900 dark:text-white'}`}>{value}</p>
