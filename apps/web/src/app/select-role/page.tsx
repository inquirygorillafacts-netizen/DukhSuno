...
export default function SelectRolePage() {
  ...
  return (
    <div className="min-h-screen w-full flex items-center justify-center p-6 relative bg-[#fdfcff] dark:bg-[#0a0a0c]">
      ...
      <div className="w-full max-w-[480px] glass bg-white/70 dark:bg-white/5 rounded-[3.5rem] p-10 shadow-2xl space-y-10 animate-in fade-in slide-in-from-bottom-8">
        <div className="text-center space-y-4">
           ...
           <h1 className="text-3xl font-black dark:text-white tracking-tight">Choose Your Path ✨</h1>
...
const RoleCard = ({ icon, title, subtitle, onClick, color, selected }: { icon: React.ReactNode, title: string, subtitle: string, onClick: () => void, color: 'rose' | 'indigo', selected: boolean }) => (
  <div 
    ...
    className={`glass p-7 rounded-[2.5rem] border cursor-pointer flex items-center gap-5 transition-all group ${
      selected ? 'border-[#ff4d6d] bg-white ring-2 ring-[#ff4d6d]/10' : 'border-white dark:border-white/10'
    }`}
  >
