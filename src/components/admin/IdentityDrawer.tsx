            ...
            <div className={`absolute right-0 top-0 bottom-0 w-full max-w-[500px] bg-white shadow-2xl transition-transform duration-500 ease-out flex flex-col ${isOpen ? 'translate-x-0' : 'translate-x-full'}`}>
                ...
                <div className="flex-1 overflow-y-auto p-8 space-y-10 no-scrollbar">
                    ...
                    <div className="space-y-4 pt-4 border-t border-slate-100">
                        <div className="flex items-center justify-between">
                            <h3 className="text-[10px] font-black text-slate-400 uppercase tracking-[0.2em]">Administrative Access</h3>
                            <div className="flex gap-2">
                                <div className={`px-2 py-0.5 rounded-md text-[8px] font-black uppercase ${user.roles?.includes('admin') ? 'bg-indigo-100 text-indigo-600' : 'bg-slate-100 text-slate-400'}`}>
                                    {user.roles?.includes('admin') ? 'Admin User' : 'Standard User'}
                                </div>
                                {user.owner === true && (
                                    <div className="px-2 py-0.5 rounded-md text-[8px] font-black uppercase bg-rose-100 text-rose-600">
                                        Owner
                                    </div>
                                )}
                            </div>
                            ...
                            <button 
                                onClick={async () => {
                                    ...
                                    const updateData: any = {
                                        roles: isAdmin ? arrayRemove('admin') : arrayUnion('admin')
                                    };
                                    // If removing admin role, also remove owner field
                                    if (isAdmin) {
                                        updateData.owner = deleteField();
                                    }
                                    await updateDoc(doc(db, 'users', user.uid), updateData);
                                    alert(`Admin role ${isAdmin ? 'removed' : 'added'}${isAdmin ? ' (Owner status also removed)' : ''} successfully!`);
                                }}
                                className={`w-full py-4 rounded-[1.5rem] text-[10px] font-black uppercase tracking-widest flex items-center justify-center gap-2 transition-all active:scale-95 border ${
                                    user.roles?.includes('admin') 
                                    ? 'bg-white border-rose-200 text-rose-600 hover:bg-rose-50' 
                                    : 'bg-indigo-600 text-white border-indigo-500 hover:bg-indigo-700 shadow-lg shadow-indigo-100'
                                }`}
                            >
                                <ShieldCheck size={14} /> {user.roles?.includes('admin') ? 'Remove Admin Privileges' : 'Promote to System Admin'}
                            </button>
                            ...
                            {user.roles?.includes('admin') && (
                                <div className="space-y-2 pt-2">
                                    <div className="flex gap-2">
                                        <button 
                                            onClick={async () => {
                                                await updateDoc(doc(db, 'users', user.uid), { owner: true });
                                                alert('Owner status enabled (True)!');
                                            }}
                                            className={`flex-1 py-4 rounded-[1.5rem] text-[10px] font-black uppercase tracking-widest flex items-center justify-center gap-2 transition-all active:scale-95 border ${
                                                user.owner === true 
                                        ...
                                        <button 
                                            onClick={async () => {
                                                await updateDoc(doc(db, 'users', user.uid), { owner: false });
                                                alert('Owner status disabled (False)!');
                                            }}
                                            className={`flex-1 py-4 rounded-[1.5rem] text-[10px] font-black uppercase tracking-widest flex items-center justify-center gap-2 transition-all active:scale-95 border ${
                                                user.owner === false 
                                    ...
                                    <button 
                                        onClick={async () => {
                                            await updateDoc(doc(db, 'users', user.uid), { owner: deleteField() });
                                            alert('Owner field removed entirely!');
                                        }}
                                        className="w-full py-3 rounded-xl text-[9px] font-black uppercase text-slate-400 hover:text-rose-600 transition-colors"
                                    >
                                        Delete Owner Field
                                    </button>
                                </div>
