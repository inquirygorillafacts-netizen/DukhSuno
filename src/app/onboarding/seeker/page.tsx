  ...
  const handleCompleteWithAvatar = async (avatarUrl: string) => {
    ...
    try {
      const updateData = {
        ...
        avatarUrl: avatarUrl,
        activeRole: 'seeker',
        roles: arrayUnion('seeker'),
        isVerified: true,
        verificationStatus: 'verified',
        registeredAt: serverTimestamp(),
      };
      await updateDoc(doc(db, 'users', user.uid), updateData);
      setUser({ ...user, ...updateData, roles: Array.isArray(user.roles) ? [...user.roles, 'seeker'] : ['seeker'] } as any);
  ...
  const handleComplete = async () => {
    ...
    try {
      ...
      setUser({
        ...user,
        ...updateData,
        roles: Array.isArray(user.roles) ? [...user.roles, 'seeker'] : ['seeker'],
      } as any);
