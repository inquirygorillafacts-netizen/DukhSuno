                ...
                <div className="relative z-10 flex flex-col md:flex-row md:items-center justify-between gap-8">
                    <div className="max-w-xl">
                        <h1 className="text-4xl font-black text-slate-900 tracking-tighter leading-none mb-4 italic uppercase">
                            Find Professional <span className="text-rose-600">Guidance</span> ❤️
                        </h1>
                        <p className="text-sm text-slate-500 font-medium tracking-tight leading-relaxed">
                            Connect with verified experts. Our professional counselors are ready to support your journey.
                        </p>
                    </div>
                ...
                <div className="flex items-center justify-between pt-4 border-t border-slate-50 mt-auto">
                    <div className="flex items-center gap-4">
                        ...
                        <div className="flex items-center gap-1 text-slate-400">
                            ...
                            <span className="text-[11px] font-black">{listener.totalSessions || "100"}+</span>
                        </div>
                    </div>
                    <div className="text-right">
                        <p className="text-[9px] font-black text-slate-400 uppercase tracking-widest leading-none mb-1">Professional Rate</p>
                        <p className="text-xl font-black text-rose-600 tracking-tighter leading-none italic">₹{listener.cheapestPlan?.price || "10"}/m</p>
                    </div>
                </div>

                <Link 
                    href={`/p/${listener.uid}`}
                    className="w-full h-14 bg-slate-900 text-white rounded-2xl flex items-center justify-center gap-2 text-[11px] font-black uppercase tracking-[0.2em] hover:bg-rose-600 transition-colors shadow-xl active:scale-95"
                >
                    <Zap size={14} className="group-hover:animate-pulse" /> Consult Now
                </Link>
            </div>
        </div>
    );
